  ...
  return (
    <div className="w-full bg-stone-50 min-h-screen">
      ...
      <nav
        ...
        className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-2 text-xs text-stone-500 flex items-center flex-wrap gap-1.5"
      >
        <Link
          href="/"
        ...
        {post.category ? (
          <>
            <Link
              ...
              className="hover:text-pink-600 font-medium transition-colors"
            >
              {post.category.name}
            </Link>
            {post.subCategory && (
        ...
        <ChevronRight className="w-3 h-3 text-stone-400" />
        <span className="text-stone-800 font-medium truncate max-w-[200px] sm:max-w-[350px]">
          {post.title}
        </span>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        ...
        {/* 3. Bottom More Stories Row (Continuous Discovery) */}
        {recommendations.length > 0 && (
          <section className="mt-16 pt-10 border-t border-stone-300">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl sm:text-2xl font-serif font-bold text-stone-950">
                More Stories from Sereia
              </h3>
              <Link
                href="/"
                className="text-xs font-bold text-pink-600 hover:text-pink-700 uppercase tracking-wider transition"
              >
                View Gazette &rarr;
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5">
              {recommendations.slice(0, 4).map((rel) => (
                <Link
                  key={rel.id}
                  href={`/${rel.slug}`}
                  className="group block bg-white rounded-xl border border-stone-200 overflow-hidden hover:shadow-md transition-shadow flex flex-col"
                >
                  ...
                  <div className="p-3.5 flex-1 flex flex-col justify-between">
                    {rel.category && (
                      <span className="text-[10px] font-bold uppercase tracking-wider text-pink-600 mb-1">
                        {rel.category.name}
                      </span>
                    )}
                    <h4 className="font-serif font-bold text-xs sm:text-sm text-stone-900 group-hover:text-pink-600 transition-colors line-clamp-2 leading-snug">
                      {rel.title}
                    </h4>
