...
export function getCategoryUrl(category: Pick<Category, 'slug' | 'parentId'> & { id?: string }, categories: Category[]): string {
  if (!category || !category.slug) return '/';
  
  const cleanSlug = category.slug.replace(/^\/+|\/+$/g, '').trim();
  ...
  if (parent && parent.slug) {
    const cleanParentSlug = parent.slug.replace(/^\/+|\/+$/g, '').trim();
