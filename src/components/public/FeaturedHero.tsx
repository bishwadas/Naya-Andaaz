            ...
            {supportingPosts.map((post) => {
              ...
              return (
                <div key={`mob-hero-${post.id}`} className="group flex items-center gap-3.5 sm:gap-4" id={`mob-hero-item-${post.id}`}>
                  <Link href={`/${post.slug}`} className="w-28 sm:w-36 aspect-[16/9] shrink-0 overflow-hidden bg-gray-100 block" id={`mob-hero-img-link-${post.id}`}>
                    <img
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Link href={`/${post.slug}`} className="hover:text-pink-600 transition-colors block" id={`mob-hero-title-link-${post.id}`}>
          ...
          {supportingPosts.map((post) => {
            ...
            return (
              <div key={`desktop-hero-${post.id}`} className="group flex flex-col" id={`desktop-hero-item-${post.id}`}>
                <Link href={`/${post.slug}`} className="block aspect-[16/9] w-full overflow-hidden bg-gray-100" id={`desktop-hero-img-link-${post.id}`}>
                  <img
                    src={post.featuredImage || 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?auto=format&fit=crop&w=1200&q=80'}
                    alt={post.title}
                    width="1200"
                ...
                <div className="mt-2 flex-1 flex flex-col">
                  ...
                  <Link href={`/${post.slug}`} className="hover:text-pink-600 transition-colors block" id={`desktop-hero-title-link-${post.id}`}>
