  ...
  return (
    <section className="py-6 sm:py-8 border-b border-gray-200 last:border-b-0" id={`section-${categorySlug}`}>
      ...
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        <Link
          href={`/${categorySlug}`}
          className="group flex items-center gap-2 text-xs sm:text-sm font-bold uppercase tracking-wider text-gray-900 hover:text-pink-600 transition-colors"
        ...
        {posts.map((post) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <span className="text-pink-600 text-[11px] sm:text-xs font-semibold uppercase tracking-wider block mb-0.5 pointer-events-none">
                {post.subCategory?.name || post.category?.name || title}
              </span>
              <Link
                href={`/${post.slug}`}
            ...
            {leftPosts.map((post) => (
              <div key={`left-${post.id}`} className="group block" id={`left-card-${post.id}`}>
                ...
                <div className="mt-2.5">
                  <span className="text-pink-600 text-xs font-bold uppercase tracking-wider block mb-0.5 pointer-events-none">
                    {post.subCategory?.name || post.category?.name || title}
                  </span>
                  <Link
                    href={`/${post.slug}`}
          ...
          <div className={`${leftPosts.length > 0 && rightPosts.length > 0 ? 'lg:col-span-6' : leftPosts.length > 0 || rightPosts.length > 0 ? 'lg:col-span-8' : 'lg:col-span-12'} flex flex-col`}>
            <div className="group block h-full flex flex-col justify-between" id={`center-card-${centerPost.id}`}>
              ...
              <div className="mt-3 flex-1 flex flex-col justify-start">
                <span className="text-pink-600 text-xs sm:text-sm font-bold uppercase tracking-wider block mb-1 pointer-events-none">
                  {centerPost.subCategory?.name || centerPost.category?.name || title}
                </span>
                <Link
                  href={`/${centerPost.slug}`}
                  className="hover:text-pink-600 transition-colors block"
