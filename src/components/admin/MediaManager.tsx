  ...
  const handleCreateMedia = async (e: React.FormEvent) => {
    ...
    try {
      if (uploadMode === 'file') {
        ...
        formData.append('folder', 'sereia_cms');
      ...
      setErrorMsg(err.message || 'An error occurred during media submission.');
    } finally {
      setIsSubmitting(false);
    }
  };
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-xl max-w-md w-full p-6 space-y-4">
            ...
            <form onSubmit={handleCreateMedia} className="space-y-4 text-xs">
              ...
              {uploadMode === 'file' ? (
                <div>
                  <label className="block text-stone-700 font-semibold mb-1">
                    Select Image File * (PNG, JPG, WebP, SVG, GIF, AVIF up to 10MB)
                  </label>
                  <input
                    type="file"
                    accept="image/*"
                ...
                <div>
                  ...
                  />
                  <p className="text-[10px] text-stone-400 mt-1">
                    Provide an external CDN image link or static media URL.
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Photo by Naya Andaaz Editorial Studio"
                  value={caption}
