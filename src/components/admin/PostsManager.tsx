...
export const PostsManager: React.FC<PostsManagerProps> = ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#FFFDFC] border border-[#E5E0DA] rounded-2xl p-5 shadow-xs">
            ...
            <div className="flex items-center flex-wrap gap-2.5">
              ...
              <button
                type="button"
                onClick={() => router.push('/admin/posts/categories')}
                className="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-[#DDD6D0] hover:border-[#EC008C] hover:text-[#EC008C] text-[#6B625C] text-xs font-medium rounded-xl transition shadow-xs"
              >
      ...
      {/* VIEW: Tags Manager (Redesigned matching Image 2) */}
      {viewMode === 'tags' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between pb-1">
            <button
              ...
              className="px-3.5 py-1.5 bg-white border border-stone-200 hover:border-stone-800 text-stone-800 text-xs font-bold rounded-xl transition shadow-2xs flex items-center gap-1.5 cursor-pointer"
            >
              <ChevronLeft className="w-3.5 h-3.5" />
              <span>Back to Posts List</span>
            </button>
          </div>
          <TagsManager onTagsUpdated={onPostsUpdated} />
        </div>
      )}
    </div>
  );
